...
const inputChecker = (req: express.Request, res : express.Response, next:Function): void =>{
  ...
  const imagePath = path.normalize(`../image-processing-api/images/fullSize/${imageName}`);
  ...
  if (Number.isNaN(width) || Number.isNaN(height) ) {
    ...
    res.status(501).send(`<div>The Width and Height must be a number greather than 0</div>`);
    return;
  } else {
    next();
  }
